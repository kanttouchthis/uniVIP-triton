...
def train(ppl, dataset_cfg_dict, optimizer_cfg_dict):
    ...
    if LOCAL_RANK == 0:
        ...
        if not RESUME:
            evaluate_pred(ppl, step, val_data, writer_val, is_write)
            evaluate_interp(ppl, step, val_data, writer_val)
    ...
    while step <  total_step+1:
        ...
        for data in train_data:
            ...
            ori_img0 = data_gpu[:, :3]
            ori_img1 = data_gpu[:, 3:6]
            ori_img2 = data_gpu[:, 6:9]
            learning_rate = get_learning_rate(total_step, step, init_lr, min_lr)

            # frame interpolation training
            img0 = ori_img0
            gt = ori_img1
            img1 = ori_img2
            time_period = 0.5
            ...
            if step % 100 == 1 and LOCAL_RANK == 0:
                writer.add_scalar(
                        '1-loss_interp_l2', extra_dict["loss_l2"] , step)
            ...
            if step % 100 == 1 and LOCAL_RANK == 0:
                writer.add_scalar(
                        '1-loss_pred_l2', extra_dict["loss_l2"] , step)
            if step % 1000 == 1 and LOCAL_RANK == 0:
                gt = (gt.permute(0, 2, 3, 1).detach().cpu().numpy() * 255)\
                        .astype('uint8')
                pred = (pred.permute(0, 2, 3, 1).detach().cpu().numpy() * 255)\
            ...
            if LOCAL_RANK == 0:
                print("{} => train step: {}/{}; time: {:.2f}+{:.2f}; "\
                        "loss_pred_l2: {:.4e}".format(
                            EXP_NAME, step, total_step,
                            data_time_interval, train_time_interval,
                            extra_dict["loss_l2"]))

            if (LOCAL_RANK == 0) and (step % save_interval == 0):
                evaluate_pred(ppl, step, val_data, writer_val)
                evaluate_interp(ppl, step, val_data, writer_val)
                ppl.save_model(model_log_dir, LOCAL_RANK)
...
def evaluate_pred(ppl, step, val_data, writer_val, is_write=True):
    psnr_list = []
    ...
    for i, data in enumerate(val_data):
        ...
        gt = data_gpu[:, 6:9]
        with torch.no_grad():
            pred, _ = ppl.model(img0, img1, time_period=2)
        ...
        time_stamp = time.time()
        if LOCAL_RANK == 0:
            print('{} => PREDICTION val step: {}: {}/{}; time: {:.2f}+{:.2f}'\
    ...
    if LOCAL_RANK == 0:
        ...
        if is_write:
            writer_val.add_scalar('0-pred-psnr', psnr, step)
        logger.info('{} => PREDICTION val step: {}; psnr: {:.4f}'.format(
            EXP_NAME, step, psnr))

def evaluate_interp(ppl, step, val_data, writer_val, is_write=True):
    psnr_list = []
    start_time_stamp = time.time()
    time_stamp = start_time_stamp
    nr_val = val_data.__len__()
    for i, data in enumerate(val_data):
        ...
        img1 = data_gpu[:, 6:9]
        with torch.no_grad():
            pred, _ = ppl.model(img0, img1, time_period=0.5)
        for j in range(gt.shape[0]):
            this_gt = gt[j]
            this_pred = pred[j]
            psnr = -10 * math.log10(
        ...
        time_stamp = time.time()
        if LOCAL_RANK == 0:
            print('{} => INTERPOLATION val step: {}: {}/{}; time: {:.2f}+{:.2f}'\
                    .format(EXP_NAME, step, i, nr_val,\
                    data_time_interval, eval_time_interval))
    ...
    if LOCAL_RANK == 0:
        ...
        psnr = np.array(psnr_list).mean()
        if is_write:
            writer_val.add_scalar('0-interp-psnr', psnr, step)
        logger.info('{} => INTERPOLATION val step: {}; psnr: {:.4f}'.format(
            EXP_NAME, step, psnr))
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
            description='train uniVIP for unified video frame interpolation and prediction')

    # => args for basic information
    parser.add_argument('--exp_name', default="univip-base", type=str,
    ...
    parser.add_argument('--resume', default=False, type=strtobool,
    ...
    parser.add_argument('--large_model', type=strtobool, default=False,
            help='if False, train uniVIP-B; if True, train uniVIP-L model')
    parser.add_argument('--load_pretrain', type=strtobool, default=False,
            help='whether load pre-trained weight')
    parser.add_argument('--model_file', type=str, default="",
            help='weight of uniVIP')
